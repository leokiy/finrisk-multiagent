...
| 阶段 | 做什么 | 并行/串行 |
|------|--------|:--:|
| **第零轮** | 为每个 Agent 生成针对其角色的专属分析策略 | 串行 |
| **第一轮** | 数据提取、风险评估、合规审查 3 个 Agent 并行执行，各用自己的专属转述问题，可联网搜索 | 并行 ⚡ |
| **第二轮** | 深度质疑 Agent 审阅前三方输出，系统性挑战结论、寻找盲点和矛盾 | 串行 |
| **第三轮** | 第一轮 Agent 收到质疑后做二次回应——质疑是否合理？分析需要调整吗？ | 并行 ⚡ |
| **第四轮** | 协调 Agent 综合原始分析 + 质疑 + 回应，裁决矛盾，流式生成最终报告 | 串行 |
